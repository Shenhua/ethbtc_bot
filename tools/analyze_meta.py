#!/usr/bin/env python3
"""
tools/analyze_meta.py - Meta Strategy Diagnostic Tool

This script analyzes the CSV output from a Meta Strategy backtest to verify that
the regime switching logic (between Mean Reversion and Trend) functioned correctly.
It checks if the final `target_w` matches the expected sub-strategy signal based
on the `regime_score` and threshold.
"""
import pandas as pd
import numpy as np
import argparse
...
def analyze(csv_path, threshold=25.0):
    """
    Analyzes a backtest CSV file to verify regime switching logic.

    Args:
        csv_path: Path to the CSV file generated by the backtester.
        threshold: The ADX threshold used for regime switching.
    """
