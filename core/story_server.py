...
This module extends the standard Prometheus metrics server to also serve a
real-time, human-readable "Story" of the trading bot's operation via HTTP.
It provides a web interface to view the log file generated by StoryWriter.
"""
...
class StoryMetricsHandler(MetricsHandler):
    """
    Extended Prometheus handler that also serves the /story endpoint.
    Inherits from prometheus_client.MetricsHandler to keep /metrics functionality.
    """
    ...
    def do_GET(self):
        ...
        - /story: Serves the story log (HTML or JSON).
        - /: Serves a simple index page.
        - /metrics: Serves Prometheus metrics (handled by superclass).
        """
    ...
    def serve_index(self):
        """
        Serves a simple HTML index page with links to available endpoints.
        """
        html = """<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    def serve_story(self, parsed_url):
        """
        Serves the story log content in HTML or JSON format.
    ...
    def _read_story_lines(self, filepath: str, max_lines: int, filter_type: Optional[str]) -> List[str]:
        """
        Reads the last N lines from the story file, optionally filtering content.

        Args:
            filepath: Path to the log file.
            max_lines: Maximum number of lines to return.
            filter_type: String to filter lines by (case-insensitive).

        Returns:
            List[str]: List of filtered lines.
        """
        try:
    ...
    def _serve_html(self, lines: List[str]):
        """
        Renders the story lines as a styled HTML page with auto-refresh.

        Args:
            lines: List of story lines to render.
        """
...
<body>
    ...
    def _serve_json(self, lines: List[str]):
        """
        Renders the story lines as a structured JSON response.

        Args:
            lines: List of story lines to parse.
        """
        import json
    ...
    def _detect_event_class(self, line: str) -> str:
        """
        Determines the CSS class for a log line based on its content (emoji or keywords).
    ...
    def _parse_story_line(self, line: str) -> Optional[Dict]:
        ...
        Returns:
            Dict or None: Structured data (timestamp, icon, event, details).
        """
        line = line.strip()
    ...
    def _detect_event_type(self, icon: str, event: str) -> str:
        """
        Helper to map icon/event text to a machine-readable event type string.
        """
        mapping = {
